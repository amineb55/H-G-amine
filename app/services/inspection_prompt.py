...
The catalogs live in ``app/rules/`` as one YAML file per rule set and the
...
class Rule(BaseModel):
    """One auditable rule of a rule set."""
...
class RuleCatalog(BaseModel):
    """The set of rules applied to one rule set."""

    referentiel: Referentiel = Field(..., description="Rule set these rules belong to.")
    label: str = Field(..., min_length=1, description="Human name shown to readers.")
    description: str = Field("", description="One line describing what the rule set covers.")
    rules: list[Rule] = Field(..., min_length=1, description="Rules to audit against.")


@lru_cache
def load_catalog(referentiel: str) -> RuleCatalog:
    """Load and validate the catalog of a rule set.

    The rule set is checked against the known values before it is used to
    build a path, so it can never reach outside the rules directory.
    """
    try:
        known = Referentiel(referentiel)
    except ValueError as exc:
        raise PromptError(f"Unknown rule set: '{referentiel}'.") from exc

    catalog_path = RULES_DIR / f"{known.value}.yaml"
    try:
        raw = yaml.safe_load(catalog_path.read_text(encoding="utf-8"))
    except FileNotFoundError as exc:
        raise PromptError(f"No rule catalog for rule set '{known.value}'.") from exc
    ...
    if catalog.referentiel != known:
        raise PromptError(
            f"Rule catalog file '{catalog_path.name}' declares rule set "
...
def referentiel_label(referentiel: str) -> str:
    """The human name of a rule set, for anything a person reads.
    ...
    try:
        ...
        logger.warning("No catalog for rule set '%s'; showing the raw key", referentiel)
        return referentiel


def referentiel_description(referentiel: str) -> str:
    """One line describing what a rule set covers, for the entry page."""
    try:
...
def rule_ids(referentiel: str) -> set[str]:
    """Rule identifiers belonging to a rule set, for checking a response."""
    try:
...
def build_system_prompt(referentiel: str) -> str:
    """Build the audit prompt for a rule set, catalog included."""
